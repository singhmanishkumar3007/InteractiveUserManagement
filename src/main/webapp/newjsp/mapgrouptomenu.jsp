...
<script type="text/javascript">
	function docSubmit() {
		...
		var selection_group_id_menu=document.getElementById('groupNameForMappingWithMenu').value;
		var select2 = document.getElementById('select2').value;
		var InvForm = document.forms.mapGroupToMenu;
		var selMenuVal = "";
		var x = 0;
		for (x = 0; x < InvForm.select2.length; x++) {
			if (InvForm.select2[x].selected) {
				//alert(InvForm.kb[x].value);
				selMenuVal = selMenuVal + InvForm.select2[x].value + ",";
			}
		}
		document.forms["mapGroupToMenu"].action = "mapgrouptomenu.action?groupNameWithMenu="+ selection_group_id_menu+ "&checkedMenuItems="+ selMenuVal;
			...
			<div class="row">
				...
				<div class="allcontant-first">
					<div class="allheadingstyle">
						<div class="row">
							<div class="col-sm-6">
								<h3>Please provide below information to map Menu to Group</h3>
							</div>
						</div>
					</div>
							...
							<div class="list-menu">
								...
								<select multiple id="select1" cssClass="list-menu">
									<c:forEach items="${subMap}" var="menuList" varStatus="count">
										<li>
											<p>
												<option value="${menuList.key}"><c:out
		...
		<s:if test="showResult=='yes'">
			...
			<div id="result">
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-md-10">Menu Mapping to User Group done
							successfully</div>
					</div>
				</div>
				<hr>

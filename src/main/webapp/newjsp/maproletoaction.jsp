...
<title>User Management-Role-Map Role To Action</title>
<link
...
<script type="text/javascript">
	function docSubmit() {
		...
		var InvForm = document.forms.mapRoleToAction;
		var SelBranchVal = "";
		var x = 0;
		for (x = 0; x < InvForm.select2.length; x++) {
			if (InvForm.select2[x].selected) {
				//alert(InvForm.kb[x].value);
				SelBranchVal = SelBranchVal + InvForm.select2[x].value + ",";
			}
		}
		document.forms["mapRoleToAction"].action = "maproletoaction.action"
		/* ?groupNameForMappingWithMenu="
				+ selection_group_id
				+ "&checkedMenuItems="
				+ checkedMenuItems */

		document.forms["mapRoleToAction"].submit();
	}
</script>
...
<body>
	...
	<div class="container">
		<s:form name="mapRoleToAction" action="maproletoaction"
			id="mapRoleToAction">
			<!-- container-->
			<div class="row">
				<!-- row-->
				...
				<div class="allcontant-first">
					<div class="allheadingstyle">
						<div class="row">
							<div class="col-sm-6">
								<h3>Please provide below information to map Role to Action</h3>
							</div>
						</div>
					</div>
					<div class="border-style">
						<div class="row">
							<div class="col-sm-6 pdtwo">
								<div class="row">
									<div class="col-sm-5">
										<label class="sh-label">Select Role</label>
									</div>
									<div class="col-sm-7">
										<select name="groupNameForMappingWithMenu"
											id="groupNameForMappingWithMenu" class="sh-select">
								...
								<div class="row">
									...
									<div class="col-sm-6 col-xs-6">
										<div class="onofftwoswitch">
											<input type="checkbox" name="onofftwoswitch"
												class="onofftwoswitch-checkbox" id="myonofftwoswitch">
											<label class="onofftwoswitch-label" for="myonofftwoswitch">
												<span class="onofftwoswitch-inner"></span> <span
												class="onofftwoswitch-switch"></span>
											</label>
										</div>
									</div>
								</div>
				...
				<div class="col-sm-12 maptogroupbt">
					<button class="sh-input btn" onclick="docSubmit();">Map
						role to action</button>
				</div>
				<div class="heading-two">
					<!-- heading-one-->
					<h3>Map Role to Action</h3>
				</div>
				<!-- heading-one-->
						...
						<div class="col-sm-5">
							<div class="list-menu">
								...
								<select multiple id="select1" cssClass="list-menu">
									<c:forEach items="${subMap}" var="menuList" varStatus="count">
										<li>
											<p>
												<option value="${menuList.value}"><c:out
														value="${menuList.value}" /></option>
											</p>
										</li>
		...
		<s:if test="showResult=='yes'">
			...
			<div id="result">
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-md-10">Action Mapping to Role  done
							successfully</div>
					</div>
				</div>
				<hr>
